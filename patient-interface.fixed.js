// patient-interface.fixed.js — clean patient dashboard script
...
(function(){
  ...
  function loadDoctors(){
    fetch('doctors.json').then(function(r){ return r.json(); }).then(function(data){
      ...
      populateDoctors([ { id: '1', name: 'Dr. Sarah Thompson', specialty: 'Cardiology' }, { id: '2', name: 'Dr. James Patel', specialty: 'Neurology' } ]);
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    var displayName = patient.name || patient.firstName || patient.username || patient.email || 'Patient';
    ...
    var bookBtn = $id('book-btn'); if (bookBtn) bookBtn.addEventListener('click', function(){
      ...
      var ap = { id: Date.now(), userEmail: patient.email||patient.username||displayName, doctorId: doctorId, doctorName: doctorName, date: date, time: time, status: 'Scheduled', createdAt: new Date().toISOString() };
      apps.push(ap); saveAppointments(apps);
